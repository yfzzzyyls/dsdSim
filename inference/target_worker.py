...
class SpeculativeService(speculative_pb2_grpc.SpeculativeServiceServicer):
    ...
    def NextToken(self, request, context):
        ...
        if prompt:
            ...
            if self.input_ids is None:
                ...
                return speculative_pb2.NextTokenResponse()
            seq = self.model.sample(self.input_ids, sequence_length=self.input_ids.shape[1] + 1)
        ...
        return speculative_pb2.NextTokenResponse(token_id=token_id, token_text=token_text)

def run_server(model_name: str, port: int = 50051):
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=1))
    speculative_service = SpeculativeService(model_name)
    speculative_pb2_grpc.add_SpeculativeServiceServicer_to_server(speculative_service, server)
