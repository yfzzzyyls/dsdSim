...
from grpc_comm import inference_pb2, inference_pb2_grpc  # Updated imports for grpc files
import time
import json
...
def run_local(model_path, prompt: str, max_new_tokens=50, sequence_length=128, profile=False):
    ...
    for i in range(max_new_tokens):
        ...
        token_id = int(output[0][-1]) if isinstance(output, (list, tuple)) else int(output[0, -1])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, required=True, help="Path to the target model (original checkpoint or compiled folder)")
    parser.add_argument("--port", type=int, default=50051, help="Port for the gRPC server")
    parser.add_argument("--sequence_length", type=int, default=128, help="Sequence length for model compilation (if needed)")
