...
def speculative_decode(draft_model, tokenizer, target_stub, prompt_text: str, max_new_tokens: int = 50, window_size: int = 4):
    ...
    This version uses the GenerateFull RPC (which takes a GenerateRequest and returns a GenerateResponse)
    to obtain one token at a time from the target. It then uses the tokenizer to convert the token text
    into a token id for comparison with the draft model's prediction.
    ...
    # Obtain the first token from the target.
    ...
    # Convert token text to a token id.
    ...
    while tokens_generated < max_new_tokens:
        ...
        if isinstance(draft_seq, (list, tuple)):
            # If sample returns a list, take the last element of the first entry.
            draft_token_id = int(draft_seq[0][-1]) if isinstance(draft_seq[0], (list, tuple)) else int(draft_seq[0])
        else:
            # Assume sample returns a tensor.
        ...
        # Append the accepted token to input_ids.
        new_token_tensor = torch.tensor([[target_token_id]], dtype=input_ids.dtype)
        input_ids = torch.cat([input_ids, new_token_tensor], dim=1)
