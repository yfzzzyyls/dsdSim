import logging
import torch

logger = logging.getLogger(__name__)

def speculative_decode(draft_model, tokenizer, target_stub, prompt_text: str,
                       max_new_tokens: int = 50, profile_data=None):
    """
    Perform distributed speculative decoding using the draft model and target model via gRPC.
    If 'profile_data' is provided, we store tokens_generated and match_count,
    but we do NOT measure time inside this function to avoid overhead.

    The caller will measure total time by start_time = time.time() before calling us,
    end_time = time.time() afterwards.
    """
    from grpc_comm import inference_pb2
    input_ids = tokenizer(prompt_text, return_tensors='pt').input_ids
    output_text = ""
    logger.info("Sending initial prompt to target for first token...")

    # Request the first token from the target
    request = inference_pb2.GenerateRequest(prompt=prompt_text, max_new_tokens=1)
    response = target_stub.GenerateFull(request)
    target_token_text = response.output_text
    target_token_ids = tokenizer.encode(target_token_text, add_special_tokens=False)
    if not target_token_ids:
        logger.error("Target server returned empty token text.")
        return output_text

    target_token_id = target_token_ids[0]
    output_text += target_token_text
    new_token_tensor = torch.tensor([[target_token_id]], dtype=input_ids.dtype)
    input_ids = torch.cat([input_ids, new_token_tensor], dim=1)

    logger.info(f"Received first token from target: {repr(target_token_text)} (id {target_token_id})")

    tokens_generated = 1
    match_count = 0

    # Generate the rest
    while tokens_generated < max_new_tokens:
        # Draft predicts next token
        draft_seq = draft_model.sample(input_ids, sequence_length=input_ids.shape[1] + 1)
        if isinstance(draft_seq, (list, tuple)):
            draft_token_id = int(draft_seq[0][-1])
        else:
            draft_token_id = int(draft_seq[0, -1])

        draft_token_text = tokenizer.decode([draft_token_id], clean_up_tokenization_spaces=True)

        # Query target for the next token
        request = inference_pb2.GenerateRequest(prompt="", max_new_tokens=1)
        response = target_stub.GenerateFull(request)
        target_token_text = response.output_text
        target_token_ids = tokenizer.encode(target_token_text, add_special_tokens=False)
        if not target_token_ids:
            logger.error("Target server returned empty token text.")
            break
        target_token_id = target_token_ids[0]

        # Check match
        if target_token_id == draft_token_id:
            match_count += 1
            logger.info(f"Draft predicted: {repr(draft_token_text)} (correct)")
            output_text += target_token_text
        else:
            logger.info(f"Draft predicted {repr(draft_token_text)} but target returned {repr(target_token_text)}. Accept target.")
            output_text += target_token_text

        # Append the accepted token to input
        new_token_tensor = torch.tensor([[target_token_id]], dtype=input_ids.dtype)
        input_ids = torch.cat([input_ids, new_token_tensor], dim=1)
        tokens_generated += 1

    # If profiling data is requested, store the final counts
    if profile_data is not None:
        profile_data['tokens_generated'] = tokens_generated
        profile_data['match_count'] = match_count

    return output_text
