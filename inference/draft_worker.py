import logging
import grpc
from grpc_comm import inference_pb2_grpc  # Updated import
...
def run_client(draft_model_name: str, target_host: str = "localhost", port: int = 50051,
    ...
    if no_target:
        ...
        if profile:
            ...
            throughput = tokens_generated / total_time if total_time and total_time > 0 else float('inf')
            logger.info(f"Draft-only generation completed in {total_time:.2f} seconds.")
            logger.info(f"Tokens: {tokens_generated}, Throughput: {throughput:.2f} tokens/sec, Avg token time: {avg_time:.4f} sec")
    ...
    # For distributed speculative decoding mode:
    ...
    if profile:
        ...
        match_rate = (match_count / (tokens_generated - 1)) if tokens_generated > 1 else None
        avg_time = total_time / tokens_generated if tokens_generated > 0 else 0.0
        ...
        logger.info(f"Tokens: {tokens_generated}, Token match rate: {'N/A' if match_rate is None else f'{match_rate*100:.1f}%'}, Throughput: {throughput:.2f} tokens/sec, Avg token time: {avg_time:.4f} sec")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, required=True, help="Draft model path (original checkpoint or compiled folder)")
    parser.add_argument("--target_host", type=str, default="localhost", help="Target server host address")
    ...
    parser.add_argument("--target_model", type=str, help="Path or name of target model (for tokenizer compatibility; optional)")
    parser.add_argument("--max_new_tokens", type=int, default=50, help="Maximum number of new tokens to generate")
    parser.add_argument("--sequence_length", type=int, default=128, help="Sequence length for model compilation (if not already compiled)")
    parser.add_argument("--profile", action="store_true", help="Enable performance profiling")
    parser.add_argument("--no_target", action="store_true", help="Run draft model standalone (without contacting target)")
