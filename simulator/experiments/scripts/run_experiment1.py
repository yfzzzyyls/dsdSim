...
BASE_CONFIG = CONFIG_ROOT / "explorer" / "baseline.yaml"
OUTPUT_ROOT = REPO_ROOT / "experiments" / "results" / "experiment1"
...
def _run_sim(config_path: Path) -> Dict[str, float]:
    cmd = ["python", str(REPO_ROOT / "simulator" / "sim.py"), "--config", str(config_path)]
