...
RESULTS_DIR = REPO_ROOT / "experiments" / "results" / "homogeneous_draft"
...
def generate_config(num_drafts: int, output_path: str | Path | None = None, batch_window_ms: float = 10.0) -> dict:
    """Generate config with N identical drafts and specified batch window."""
    
    # Load template
    template_path = CONFIG_DIR / 'homogeneous_draft.yaml'
...
def run_experiment(num_drafts: int, batch_window_ms: float = 10.0) -> Tuple[float, dict]:
    ...
    temp_config_path = RESULTS_DIR / f'temp_config_{num_drafts:02d}_drafts.yaml'
    temp_config_path.write_text(yaml.dump(config, default_flow_style=False))
    
    try:
        # Run simulation
        cmd = ['python', str(SIM_PATH), '--config', str(temp_config_path)]
...
def plot_results(results: List[dict], output_dir: Path) -> plt.Figure:
...
def run_batch_window_comparison():
    ...
    plot_path = output_dir / 'scaling_curve_100ms_comparison.png'
    ...
    for results, window in [(results_10ms, '10ms'), (results_100ms, '100ms')]:
        json_path = output_dir / f'homogeneous_draft_{window}_results.json'
