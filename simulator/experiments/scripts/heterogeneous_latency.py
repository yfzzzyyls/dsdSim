...
OUTPUT_DIR = REPO_ROOT / "experiments" / "results" / "heterogeneous_latency"
...
def run_experiment(num_drafts: int, batch_window_ms: float = 10.0) -> Tuple[Dict, Dict]:
    ...
    temp_config_path = OUTPUT_DIR / f'temp_heterogeneous_{num_drafts:02d}_drafts.yaml'
    ...
    try:
        ...
        result = subprocess.run(
            ['python', str(SIM_PATH), '--config', str(temp_config_path)],
            capture_output=True,
            text=True,
            timeout=120
        )
...
def plot_results(results: List[Dict], output_dir: Path):
    ...
    plot_path = output_dir / 'heterogeneous_latency_results.png'
    plt.savefig(plot_path, dpi=150)
    print(f"\\nPlot saved to {plot_path}")
    
    return fig

def plot_latency_distribution(latency_distributions: List[Dict], output_dir: Path):
    ...
    plot_path = output_dir / 'draft_latency_distribution.png'
    plt.savefig(plot_path, dpi=150)
...
def main():
    ...
    dist_file = OUTPUT_DIR / 'latency_distributions.json'
    dist_file.write_text(json.dumps(latency_distributions, indent=2))
