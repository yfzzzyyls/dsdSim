...
def main():
    ...
    parser.add_argument("--port", type=int, default=50051,
                        help="Port for gRPC server (target role) or for client connection (draft role)")
    ...
    if args.role == "compile":
        ...
        if model_name is None:
            ...
            return
        # If profiling with prompt provided, run standalone target generation for profiling
        ...
        verify.run_model(model_name, prompt=prompt, max_tokens=args.max_new_tokens, sequence_length=args.sequence_length, role="target")
        ...
        prompt = args.prompt or ""
        from inference import verify
        verify.run_model(model_name, prompt=prompt, max_tokens=args.max_new_tokens, sequence_length=args.sequence_length, role="draft")
    else:
